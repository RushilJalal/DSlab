...
Node *addPolynomials(Node *poly1, Node *poly2)
{
    ...
    while (current1 != poly1 && current2 != poly2)
    {
        ...
        {
            resultHeader = insertTerm(resultHeader, current1->coefficient, current1->exponent);
            current1 = current1->next;
        }
        else if (current1->exponent < current2->exponent)
        {
            resultHeader = insertTerm(resultHeader, current2->coefficient, current2->exponent);
        ...
        else
        {
            ...
            if (sum != 0)
            {
                resultHeader = insertTerm(resultHeader, sum, current1->exponent);
            }
            current1 = current1->next;
            current2 = current2->next;
        }
...
int main()
{
    ...
    poly2Header = insertTerm(poly2Header, 5, 0);
    ...
    Node *result = addPolynomials(poly1Header, poly2Header);
